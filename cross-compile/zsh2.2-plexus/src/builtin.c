...
void printparam();
char *escsubst();
struct asgment *getasg();
void printulimit();
Cond partest();
void printcompctl();
void compctl_process();
int bin_enable();
int bin_colon();
int bin_break();
int bin_fg();
...
/* Forward declarations for builtin functions (SVR2 cc needs these) */
int bin_test(), bin_dot(), bin_colon(), bin_alias(), bin_typeset();
int bin_fg(), bin_bindkey(), bin_break(), bin_cd(), bin_compctl();
int bin_dirs(), bin_disable(), bin_print(), bin_echotc(), bin_enable();
int bin_eval(), bin_let(), bin_fc(), bin_read(), bin_getopts();
int bin_hash(), bin_kill(), bin_limit(), bin_log(), bin_pwd();
int bin_rehash(), bin_sched(), bin_set(), bin_setopt(), bin_suspend();
int bin_ttyctl(), bin_times(), bin_trap(), bin_whence(), bin_ulimit();
int bin_umask(), bin_unalias(), bin_unhash(), bin_unlimit(), bin_unset();
int bin_vared();
/* Functions returning char * in this file */
char *cd_get_dest(), *cd_do_chdir(), *cd_try_chdir(), *escsubst();
/* Functions returning other types in this file */
void niceprintf(), cd_new_pwd(), printalias(), compctl_process();
void printcompctl();
struct asgment *getasg();
Cond partest();
/* External functions */
extern void freecmdnam(), freeanode(), freestr(), freecompctl();
