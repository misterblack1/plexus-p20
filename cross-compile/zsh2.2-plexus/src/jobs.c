...
long chlds,chldu;
	...
	/* On SVR2, SIGCLD must NOT be re-installed before wait() or it
	   causes infinite recursion. Re-install it after wait() instead. */
#ifndef PLEXUS_SVR2
	signal(sig,handler);
#else
	if (sig != SIGCHLD)
...
#if defined(PLEXUS_SVR2) && defined(RESETHANDNEEDED)
