...
char *argv[];
{
	...
	time(&utime);
#else
	nb = 0L;	/* should be initialized */
	if (argv[0][0] == '\0')
		argc = getargv ("mkfs", &argv, 0);
#endif
	if(argc < 3) {
...
struct inode *par;
{
	...
	in.i_uid = getnum(); /* get owner */
	in.i_gid = getnum(); /* get group */
...
daddr_t alloc() /* get a block number from the freelist */
{
	int i;
	daddr_t bno;
...
daddr_t *ib;
{
	...
	/* writes the data block db to the new file system */
newblk(adbc, db, aibc, ib) 
int *adbc, *aibc;
char *db;
daddr_t *ib;
...
bflist()
{
	...
	filsys->s_tfree = 0;
	d = filsys->s_fsize-1;
		/* f_n is the rotatational gap in blocks */
	while(d%f_n)
		d++;
	for(; d > 0; d -= f_n)
		for(i=0; i<f_n; i++) {
...
/* ip points to inode structure, aibc is # blocks, ib is array of logical
   block numbers */
iput(ip, aibc, ib)
