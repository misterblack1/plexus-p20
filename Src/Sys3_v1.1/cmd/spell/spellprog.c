...
char notusedVERS[] = "@(#)spell.src	1.4";
...
char **argv;
{
	...
	for(;; fwrite(wrbuf,sizeof(*wrbuf),strlen(wrbuf),file)) {
		affix[0] = 0;
		wrbuf[0] = 0;
		file = found;
...
lcase:
		...
		file = stdout;
foundit:	strcat(wrbuf,affix);
		strcat(wrbuf,original);
		strcat(wrbuf,"\n");
	}
}
...
char *bp, *ep;
{
	...
	if(xflag) {
		fwrite("=",1,1,stdout);
		fwrite(bp,sizeof(*bp),ep-bp,stdout);
		fwrite("\n",1,1,stdout);
	}
	for(i=0; i<NP; i++) {
