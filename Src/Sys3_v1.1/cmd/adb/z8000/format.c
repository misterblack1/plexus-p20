...
STRING		ifp;
{
	...
	DO	fp = ifp; c = *fp;
		...
		switch(modifier) {
		    ...
		    case 'b': case 'B':
			IF outdefault == hexint THEN
				printf("%-8x", (w>>8)&LOBYTE);
			ELSE
		    ...
		    case 'i':
			symbins = FALSE;
			putins(w,ISP,0); printc(EOR); break;
...
L_INT	inkdot(incr)
{
	...
	newdot=dot+((long)incr & 0xffffL);
	if(newdot < 0) newdot = -newdot;
