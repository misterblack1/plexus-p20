...
char opmap [] = {
	...
	76,76,76,76,77,76,0,76,0,95,0,0,90,90,91,91,
	5,5,60,60,41,41,6,6,73,73,13,13,0,0,0,0,
	13,43,60,43,31,42,5,42,38,38,25,25,0,31,29,8,
	31,31,2,2,3,3,1,1,28,28,24,24,26,26,31,31,
	31,31,31,31,32,31,32,31,0,95,0,0,0,83,0,53,
	5,5,60,60,41,41,6,6,73,73,13,13,0,0,0,0,
...
char fmttab[] = {
	 0022,0022,0022,0022,0022,0022,0022,0022	/* 0 */
	,0022,0022,0022,0022,0001,0001,0007,0007
	,0222,0322,0222,0122,0222,0302,0222,0102	/* 1 */
	,0222,0022,0222,0022,0002,0242,0061,0121
	,0022,0022,0041,0041,0041,0041,0041,0041	/* 2 */
	,0041,0041,0041,0041,0022,0022,0042,0042
	,0104,0104,0044,0044,0104,0304,0007,0244	/* 3 */
	,0007,0021,0003,0003,0067,0067,0087,0087
	,0022,0022,0022,0022,0022,0022,0022,0022	/* 4 */
	,0022,0022,0022,0022,0001,0001,0007,0007
	,0222,0322,0222,0122,0222,0302,0222,0102	/* 5 */
	,0222,0022,0222,0022,0002,0242,0061,0121
	,0022,0022,0041,0041,0041,0041,0041,0041	/* 6 */
	,0041,0041,0041,0041,0022,0022,0042,0042
	,0064,0064,0024,0024,0064,0264,0022,0224	/* 7 */
	,0007,0021,0007,0007,0007,0030,0007,0111
	,0022,0022,0022,0022,0022,0022,0022,0022	/* 8 */
	,0022,0022,0022,0022,0001,0001,0072,0007
	,0222,0322,0222,0122,0222,0302,0222,0102	/* 9 */
	,0222,0022,0222,0022,0002,0007,0071,0007
	,0022,0022,0041,0041,0041,0041,0041,0041	/* A */
	,0041,0041,0041,0041,0022,0022,0101,0101
	,0021,0021,0004,0004,0022,0022,0022,0022	/* B */
	,0005,0007,0006,0006,0022,0041,0022,0007
	,0043,0043,0043,0043,0043,0043,0043,0043	/* C */
	,0043,0043,0043,0043,0043,0043,0043,0043
	,0103,0103,0103,0103,0103,0103,0103,0103	/* D */
	,0103,0103,0103,0103,0103,0103,0103,0103
	,0063,0063,0063,0063,0063,0063,0063,0063	/* E */
	,0063,0063,0063,0063,0063,0063,0063,0063
	,0023,0023,0023,0023,0023,0023,0023,0023	/* F */
...
struct aux aux7[] = {
	...
	0xb900, 92, 0x23,	/* RSVD */
	0xbf00, 92, 0x23,	/* RSVD */
	0x0e00, 92, 0x23,	/* Extended */
	0x0f00, 92, 0x23,	/* Extended */
	0x4e00, 92, 0x23,	/* Extended */
	0x4f00, 92, 0x23,	/* Extended */
	0x8f00, 92, 0x23,	/* Extended */
	0x7c00, 93, 0x28,	/* di vi,nvi */
	0x7c01, 93, 0x28,	/* di vi */
	0x7c02, 93, 0x28,	/* di nvi */
	0x7c03, 93, 0x23,	/* di (illegal) */
	0x7c04, 94, 0x28,	/* ei vi,nvi */
	0x7c05, 94, 0x28,	/* ei vi */
	0x7c06, 94, 0x28,	/* ei nvi */
	0x7c07, 94, 0x23,	/* ei (illegal) */
	0,0,0
};
...
char *fops[] = {		/* table of floating point opcodes */
	"?f", "fadd`", "fsub`", "fmul`", "fdiv`", "fneg@`", "fcp`", "?f",
	"?f", "?f", "?f", "?f", "?f", "?f", "?f", "?f"
};
...
#ifdef OVKRNL
	/* If overlay then change overlay segments if we've entered stub */
	if((magic==0430 || magic==0431) && var[VARC] != wantedovl
					&& (iq[0]&0xfff0) == 0x5e00) {
		setovmap((long)wantedovl);
		var[VARC] = wantedovl;
		dot = iq[1];
		dot &= 0xffffL;
		dotinc =0;
...
case 000000:
	if((ins & 030000)==030000) {
		if((ins & 0x0c00) == 0x0c00)
			return(RA);
		if(ins & 0360){
			if((ins&0xff0f) == 0x3900) /* kludge for ldps */
				return (IR);
			return (BA);
		} else return (RA);
	} else
...
case 040000:
	if((ins & 0xff00) == 0x7d00)	/* ldctl hack */
		return(R);
	if((ins & 0360) != 0) {
		if( (ins & 0xff0f) == 0x7900 ) return(X); /* hack for ldps! */
...
case 0140000:
 ...
 /* fix up addressing mode for special cases */
bmode(ins)
int ins;
{

	/* special case for RSVD */
	switch(ins & 0xff00) {
	case 0x3600:
	case 0x3800:
	case 0x7800:
...
/* ei & di */
case 2:		switch(iq[0] & 3) {
			case 0: printf("vi,nvi");
				break;
			case 1: printf("vi");
				break;
			case 2: printf("nvi");
