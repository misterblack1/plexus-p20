...
{	/* name [ . local ] | number | . | ^ | <var | <register | 'x | | */
	...
	THEN	readsym();
		IF lastc=='.'
		THEN	frame=endhdr[r14]&EVEN; lastframe=0; callpc=endhdr[pc];
			WHILE errflg==0
			DO  savpc=callpc;
			    findroutine(frame,-ISYM);
		...
		ELSE
			expv = symp->symv;
#ifdef OVKRNL
			expo = symp->ovnumb;	/* save overlay # of symbol */
#endif
		FI
		lp--;
	...
	THEN	readchar();
		IF symchar(0)
		THEN	lastframe=savlastf; callpc=savpc; findroutine(savframe,-ISYM);
			chkloc(savframe);
		ELSE	expv=dot;
		FI
		lp--;
