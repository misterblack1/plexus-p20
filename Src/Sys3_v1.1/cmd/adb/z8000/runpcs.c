...
runpcs(runmode, execsig)
{
	...
	DO
		...
		printf("\ncontinue %d %d\n",userpc,execsig);
#endif
		ioctl(0, TCSETA, &usrtty);	/* stty(0,&usrtty); */
		IF runmode != SINGLE
		THEN
...
bpwait()
{
	...
	ioctl(0, TCGETA, &usrtty);	/* gtty(0,&usrtty); */
	ioctl(0, TCSETA, &adbtty);	/* stty(0,&adbtty); */
	IF w == -1
	THEN pid=0;
	     errflg=BADWAIT;
	ELIF (stat & 0177) != 0177
...
readregs()
{
	...
	/* if overlaid, get ov */
/*-----------------
	We really can't execute the kernel (wish we could). Hence the ptrace
	call to get its current overlay # is not yet implemented. So for now
	don't do anything and comment out the request.
	...
	THEN	int ovno;
		...
		var[VARC] = ovno;
		((struct user *)corhdr)->u_ovdata.uo_curov = ovno;
		setovmap(ovno);
	FI
-----------------*/
#endif
