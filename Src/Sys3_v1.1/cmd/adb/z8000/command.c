...
L_INT		expv;
#ifdef OVKRNL
CHAR		expo;			/* overlay associated with symbol */
CHAR		wantedovl;		/* overlay ultimately wanted */
#endif
L_INT		adrval;
INT		adrflg;
L_INT		cntval;
...
CHAR		defcom;
{
	...
	REP
/*	expo = 0;		/* default overlay is 0 */
	IF adrflg=expr(0)
	THEN
#ifdef OVKRNL
		if(expo == 0) {			/* overlay 0. no stubs */
			var[VARC] = 0;
			setovmap((long) 0);
			wantedovl = 0;
		}
		else if((long)expo == var[VARC]) {	/* same overlay. */
			if(within(expv,txtmap.b1,txtmap.e1)) {
				var[VARC] = 0;
				setovmap((long) 0);
				wantedovl = expo;
			}
			...
			wantedovl = expo;	/* different overlay */
#endif
		dot=expv;
		ditto=dot;
	FI
	...
	switch(lastcom&STRIP) {
		...
		THEN endhdr[regptr]=shorten(dot);
		     ptrace(WUREGS,pid,ctob(USIZE)+2*regptr,endhdr[regptr]);
		ELIF (modifier=varchk(savc)) != -1
		THEN	var[modifier]=dot;
#ifdef OVKRNL
			IF modifier == VARC THEN setovmap(dot); FI
#endif
		ELSE	error(BADVAR);
		FI
		break;
