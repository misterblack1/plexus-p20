...
STRING	s; MAP *amap;
{
	...
	file=amap->ufd;
#ifdef OVKRNL
	if((magic==0430 || magic==0431) && amap==&txtmap)
		printf("%s%12t`%s' [%d]\n",s,
...
#ifdef OVKRNL
/*----------------
	Not feasible to print out current overlay of process of running
	kernel. Once adb is working may come back and try some experiments
	to see if something can be done about geting the current overlay #
	from a kernel variable. Kernel can't run anyway.
