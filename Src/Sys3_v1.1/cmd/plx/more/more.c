static char *SCCS_ID = "@(#)more.c	4.1 (Berkeley) 10/16/80";
/*
...
#else
#ifdef SYS3
#define MBIT	O_XTABS /* O_XTABS = 2 = CBREAK */
#define RAW	040
#define stty(fd,argp)	ioctl(fd,TCSETA,argp)
...
#define ret(val) retval=val;done++;break
	...
	errors = 0;
#ifndef SYS3	/* MBIT == RAW for V6 only */
...
#ifdef SYS3
	otty.c_lflag &= ~ECHO;
	if (MBIT == O_XTABS || !slow_tty)	/* O_XTABS = 2 = CBREAK */
	    otty.c_cc[VTIME] = 1;
	    otty.c_cc[VMIN] = 1;
	    otty.c_lflag &= ~(ICANON|ECHOK);
#else
	otty.sg_flags &= ~ECHO;
...
{
#ifdef SYS3
	/* O_XTABS = 2 = CBREAK  (MBIT = O_XTABS) */
	otty.c_cc[VTIME] = 1;
	otty.c_cc[VMIN] = 1;
	otty.c_lflag &= ~(ICANON|ECHOK|ECHO);
#else
	otty.sg_flags |= MBIT;
	otty.sg_flags &= ~ECHO;
#endif
...
#ifdef SYS3
	/* O_XTABS = 2 = CBREAK  (MBIT = O_XTABS) */
	otty.c_cc[VEOL] = 0; /* VTIME == VEOL */
	otty.c_cc[VEOF]  = 4; /* VMIN == VEOF */
	otty.c_lflag |= (ICANON|ECHOK|ECHO);
#else
    otty.sg_flags |= ECHO;
    otty.sg_flags &= ~MBIT;
