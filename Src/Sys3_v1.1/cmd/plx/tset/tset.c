...
**		SYS3 -- if set then use system III ioctl instead of v7.
...
# endif
	...
	if (!RepOnly)
	{
		...
		mode.c_iflag |= INPCK;	/* default is no parity checking */
		mode.c_cflag &= ~(PARENB|PARODD);
		mode.c_lflag |= ICANON;	/* default is no RAW or CBREAK */
		mode.c_cc[VEOF] = 4;
		mode.c_cc[VEOL] = 0;
		if (tgetflag("EP"))	/* may want to enable INPCK */
			mode.c_cflag |= PARENB;
		if (tgetflag("OP"))
			mode.c_cflag |= (PARENB|PARODD);
		mode.c_iflag |= ICRNL;
		...
		if (tgetflag("HD"))	/* half duplex */
			mode.c_lflag &= ~(ECHO|ECHOE|ECHOK|ECHONL);
