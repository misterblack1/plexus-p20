 ...
 * Modified by Paul Fronberg June 1982 (SYS III)
 *
...
#include <sgtty.h>
	...
	while (--argc > 0 && *argv[1] == '-') {
		...
		default:
			fprintf (stderr, "usage: bls [-1ACFRabcdfgilmnqrstux] [files]\n");
			exit(1);
		}
	}
	if (Fflg)
 ...
 * for the plexus version, we chose 1024 since these are really file
 * system blocks.  the following line is from the UCB version.
...
char *file;
{
	...
	if (rep==NULL) {
		fprintf(stderr, "bls: out of memory\n");
		nomocore = 1;
		return(NULL);
	}
	if (lastp >= &flist[NFILES]) {
		...
		lastp--;
		if (msg==0) {
			fprintf(stderr, "bls: too many files\n");
			msg++;
		}
	}
	*lastp++ = rep;
...
/*
 * No longer multiplexed files in system 3
 * Replace with FIFO file designator
 *
 *		case S_IFMPB:
 *			rep->ltype = 'M';
 *			rep->lsize = statb.st_rdev;
