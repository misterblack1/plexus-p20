...
ragain:
			...
			if (checktype(&spcl, TS_END) == 1) {
				...
				sblock.s_tfree = dfree();
				sblock.s_tinode = ifree();
				dwrite( (daddr_t) 1, (char *) &sblock);
				return;
			}
...
/*
 * The next four routines were added to place the total free
 *  block count and total free inode count into the superblock.
 *  For some reason, this was not done in the stock SYSTEM III
 *  code.
 */
...
dfree() {
	...
	dread(1L, (char *)&fsblk, sizeof(fsblk));
	i = 0;
	while(alloc())
		i++;
	return(i);
...
alloc()
{
	...
	if(b == 0)
		return(0);
	if(b<fsblk.s_isize || b>=fsblk.s_fsize) {
		printf("bad free block (%D)\n", b);
		return(0);
	}
	if(fsblk.s_nfree <= 0) {
		dread(b, (char *)&buf, sizeof(buf));
		blkno = b;
		fsblk.s_nfree = buf.df_nfree;
		for(i=0; i<NICFREE; i++)
			fsblk.s_free[i] = buf.df_free[i];
	}
	return(b);
}
...
ginode(inum) {
	...
	iblk = itod(inum);
	dread((daddr_t) iblk, xblock, BSIZE);
	return(((struct dinode *) xblock) + itoo(inum));
}
