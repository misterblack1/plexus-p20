...
freereg( p, n ) NODE *p; {
	...
	if( n&NAMASK ){
		for( j=maxa; j>=mina; --j ) if( rstatus[j]&STAREG ){
			if( usable(p,n,j) ){
if (zdebug) {
	int i;
...
usable( p, n, r ) NODE *p; {
	...
	regcount = szty(p->type) * (n&NACOUNT);
	if (regcount == 1) {
		if(( busy[r] == 0 ) && !notoff(0,r)) {
			busy[r] |= TBUSY;
			return(1);
			}
		}
	if( (n&NAMASK) && (regcount == 2) ){ /* only do the pairing for real regs */
		if( r&01 ) return(0);
...
printf("BUSIED: reg(%d), size(%d)\n",r,regcount);
	...
	if( regcount >= 3) {
		if( r&03 ) return(0);
/*
		if((busy[r]==0) && (busy[r+1]==0) && (busy[r+2]==0) && (busy[r+3]==0)) {
*/
		if (((busy[r] == 0) || shareit(p,r,n))
			&& ((busy[r+1] == 0) || shareit(p,r+1,n))
			&& ((busy[r+2] == 0) || shareit(p,r+2,n))
			&& ((busy[r+3] == 0) || shareit(p,r+3,n))) {
			busy[r] |= TBUSY;
			busy[r+1] |= TBUSY;
			busy[r+2] |= TBUSY;
			busy[r+3] |= TBUSY;
...
if (zdebug)
		printf("shareit: reg(%d), p->rval(%d), szty(p->type)(%d)\n",r,p->rval,szty(p->type));
	if( (n&(NASL|NBSL)) && ushare( p, 'L', r ) ) return(1);
	if( (n&(NASR|NBSR)) && ushare( p, 'R', r ) ) return(1);
if (zdebug)
printf("	couldn't share\n");
	return(0);
	}
...
ushare( p, f, r ) NODE *p; {
	...
	if( p->op == REG ){
		return(r==p->rval || (szty(p->type)>=2 && r==p->rval+1)
			|| (szty(p->type)==4 && ((r==p->rval+2) || (r==p->rval+2))));
		}
	return(0);
	}
...
rfree( r, t ) TWORD t; {
	...
	if( istreg(r) ){
		...
		if( szty(t) >= 2 ){
			...
			if(szty(t) == 4) {
				if (--busy[r+2] < 0) cerror( "register overfreed" );
				if (--busy[r+3] < 0) cerror( "register overfreed" );
				}
			}
		}
	}
...
rbusy(r,t) TWORD t; {
	...
	if( szty(t) >= 2 ){
		...
		if (szty(t) == 4) {
			if( istreg(r+1)) ++busy[r+2];
			if( istreg(r+1)) ++busy[r+3];
			if (r&03) cerror( "illegal register" );
			}
		}
	}
	...
	*qq = NIL;
	...
	case REG:
		if( !rtyflg ){
			...
			if( p->type == SHORT ) p->type = INT;
			else if( p->type == USHORT ) p->type = UNSIGNED;
