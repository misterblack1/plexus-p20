#
char LD[] = "@(#)ld.c 1.14 79/04/26 15:04:32";	/* sccs ident */
/*
 *  link editor for VAX
 */
...
int	Vflag;		/* system environment stamp */
	...
	for (c=1; c<argc; c++) {
		...
		if (*ap == '-') {
			...
			case 'V':
			{
				...
				if(trace)
					printf("stamp=%s\n", stmp);
				if((stampid = atoi(stmp)) < 0 || stampid > 32767)
					error(1, "bad stamp");
				break;
			}
...
/*
				...
				nflag=0;
				if (tsize!=0)
					error(1, "-i: too late, some text already loaded");
				textbase = PAGRND+1;
*/
				continue;
...
char **argv;
{
	...
	middle();
	setupout();
	if(-1==(local=(SYMBOL *)sbrk(NSYMPR*sizeof(*local))))
...
middle()
{
	 ...
	 */
	csize = 0;
	database = round(tsize+textbase, ((nflag|iflag)? PAGRND:FW));
	if (dflag || rflag==0) {
...
setupout()
{
	...
	filhdr.a_magic = nflag? NMAGIC: iflag? IMAGIC:OMAGIC;
	filhdr.a_stamp = stampid;
	filhdr.a_text =
		nflag? tsize:
			iflag? round(tsize,PAGRND)+(PAGRND+1)-sizeof(filhdr):
				round(tsize, FW);
	filhdr.a_data = iflag? round(dsize,PAGRND):dsize;
	filhdr.a_bss = bsize;
	...
	if (iflag) {
		register int i=(PAGRND+1)-sizeof(filhdr);
		while (--i>=0) putc(0,tout);
	}
}
		...
		if(Tflag) {
			...
			for(i = 0; i < sizeof cursym.sname; i++) {
				...
				if(Ttrace[j][i] == 0 || i == sizeof(cursym.sname) - 1) {
					if(filname)
						printf("%s", filname);
					if(archdr.ar_name&&archdr.ar_name[0])
						printf("(%s)", archdr.ar_name);
					printf(" %s %s", (type&TYPE)?"def of":"ref to", Ttrace[j]);
					if(type&EXTERN)
						printf(" extern");
					printf(" %s\n", Ttypnam[(type&TYPE) / 2]);
					break;
...
finishout()
{
	while (tsize&(iflag? PAGRND:FW)) {putc(0, tout); tsize++;}
	fclose(dout);
	copy(doutn);
	if (iflag) while (dsize&PAGRND) {putc(0,tout); ++dsize;}
	if (rflag) {
		fclose(trout);
		copy(troutn);
		fclose(drout);
