...
#ifdef OPTCRET
unsigned int cretlab = CRETLAB;	/* local cret labels */
int cret;
#endif
int	lastseg	= -1;
int	mydebug;
...
char **argv;
{
	...
	if (nflag) {
		...
		fprintf(stderr, "%d optimized procedures\n", nproc);
...
input()
{
	...
	for (;;) {
		...
		lastp = p;
		p->ref = 0;
		if( (p->op == DWORD || p->op == DBYTE || p->op == ASCII)
		   && p->back->op == LABEL )
			p->back->op = NLABEL;
		if (oper==EROU)
			return(1);
...
register struct node *p;
{
	...
	fprintf(stderr,"label no. = %u, code = %s\n",p->labno,p->code);
