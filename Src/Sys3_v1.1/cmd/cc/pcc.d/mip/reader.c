	...
	for( c=1; c<argc; ++c ){
		if( *(cp=argv[c]) == '-' ){
			while( *++cp ){
				...
				case 'C':  /* Special code generation options */
					switch(*++cp) {
					case 'c':
						csavallregs++;
						break;
				...
				case 'z':  /* more machine dependant debugging */
					++zdebug;
					break;
...
/*CCF stock pcc bug fix -- off by one (>) became (>=) */
		if( files >= argc ) return( nerrors );
/*CCF stock pcc bug fix -- to keep from looping forever on same file (++) */
