...
#ifdef	PNETDFS
	if ( ip->i_flag & IRMT ) {
		printf( "xfree on remote text, not there %x\n", ip );
		debug();
		return;
	}
#endif
...
#ifdef	PNETDFS
	if ( ip->i_flag & IRMT ) {
		printf( "xalloc on remote %x, not there\n", ip );
		debug();
		return;
	}
#endif
...
loop:
	...
	u.u_procp->p_textp = xp;
	xexpand(xp);
	estabur(ts, (unsigned)0, (unsigned)0, 1, RW);
...
#ifdef	PNETDFS
	if ( ip->i_flag & IRMT ) {
		printf( "xuntext on remote %x, not there\n", ip );
		debug();
		return;
	}
#endif
